...
public class FolhaDePontoControllerIntegrationTest {
	...
	@Test
	public void registroComSucesso() throws Exception {
		Momento m = new Momento();
		m.setDataHora("2020-11-16T08:00:10");
		String content = this.objectMapper.writeValueAsString(m);
		this.mockMvc.perform(post("/batidas").contentType(MediaType.APPLICATION_JSON).content(content))
				.andExpect(status().isCreated())
				.andExpect(content().string("{\"dia\":\"2020-11-16\",\"horarios\":[\"08:00:10\"]}"));
	}

	@Test
	public void registroComSucesso_segundaBatida() throws Exception {
		Momento m = new Momento();
		m.setDataHora("2020-11-16T08:00:10");
	 ...
	 @Test void registroIntervaloAlmoco() throws Exception { Momento m = new
	 Momento(); m.setDataHora("2020-11-16T08:00:00"); String content =
	 this.objectMapper.writeValueAsString(m);
	 this.mockMvc.perform(post("/batidas")
	 .contentType(MediaType.APPLICATION_JSON) .content(content))
	 .andExpect(status().isCreated()) .andExpect(content().string(
	 "{\"dia\":\"2020-11-16\",\"horarios\":[\"08:00:00\"]}"));
	 
	 m.setDataHora("2020-11-16T12:00:00"); content =
	 this.objectMapper.writeValueAsString(m);
	 this.mockMvc.perform(post("/batidas")
	 .contentType(MediaType.APPLICATION_JSON) .content(content))
	 .andExpect(status().isCreated()) .andExpect(content().string(
	 "{\"dia\":\"2020-11-16\",\"horarios\":[\"08:00:00\",\"12:00:00\"]}"));
	 
	 m.setDataHora("2020-11-16T12:10:00"); content =
	 this.objectMapper.writeValueAsString(m);
	 this.mockMvc.perform(post("/batidas")
	 .contentType(MediaType.APPLICATION_JSON) .content(content))
	 .andExpect(status().isBadRequest())
	 ...
	 @Test void realizarAlocacaoComSucesso() throws Exception { Momento m = new
	 Momento(); m.setDataHora("2020-11-16T08:00:00"); String content =
	 this.objectMapper.writeValueAsString(m);
	 this.mockMvc.perform(post("/batidas")
	 .contentType(MediaType.APPLICATION_JSON) .content(content))
	 ...
	 AlocacaoForm al = new AlocacaoForm(); al.setDia("2020-11-16");
	 al.setTempo("PT2H30M10S"); al.setNomeProjeto("Folha"); content =
	 this.objectMapper.writeValueAsString(al);
	 this.mockMvc.perform(post("/alocacoes")
	 .contentType(MediaType.APPLICATION_JSON) .content(content))
	 .andExpect(status().isCreated()) .andExpect(content().string(
	 "{\"dia\":\"2020-11-16\",\"nomeProjeto\":\"Folha\",\"tempo\":\"PT2H30M10S\"}"
	 )); }
	 
	 @Test void realizarAlocacaoSemRegistroPonto() throws Exception { AlocacaoForm
	 al = new AlocacaoForm(); al.setDia("2020-11-16"); al.setTempo("PT2H30M10S");
	 al.setNomeProjeto("Folha"); String content =
	 this.objectMapper.writeValueAsString(al);
	 this.mockMvc.perform(post("/alocacoes")
	 .contentType(MediaType.APPLICATION_JSON) .content(content))
	 .andExpect(status().isBadRequest()) .andExpect(content().
	 string("Não existem batidas de ponto para esse dia. É necessário registra-las primeiro."
	 )); }
	 
	 @Test void realizarAlocacaoComTempoMaiorQueRegistrado() throws Exception {
	 Momento m = new Momento(); m.setDataHora("2020-11-16T08:00:00"); String
	 content = this.objectMapper.writeValueAsString(m);
	 this.mockMvc.perform(post("/batidas")
	 .contentType(MediaType.APPLICATION_JSON) .content(content))
	 .andExpect(status().isCreated()) .andExpect(content().string(
	 ...
	 AlocacaoForm al = new AlocacaoForm(); al.setDia("2020-11-16");
	 al.setTempo("PT5H30M10S"); al.setNomeProjeto("Folha"); content =
	 this.objectMapper.writeValueAsString(al);
	 this.mockMvc.perform(post("/alocacoes")
	 .contentType(MediaType.APPLICATION_JSON) .content(content))
	 .andExpect(status().isBadRequest()) .andExpect(content().
	 string("A Alocação não pode ser realizada porque ultrapassou o tempo trabalhado do dia"
	 )); }
